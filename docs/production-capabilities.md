...
| Sales and business records | Total-only sale schema/API/universal UI, frozen currency exponent, IANA/wall-clock snapshots, permission-checked tenancy, transactional idempotency/audit receipt, canonical result, and previous-month summary with PostgreSQL 18 tests | Add product-route rate limits/kill switch and atomic void/replacement correction before claiming the approved first sales milestone complete; then separately brief later domains | Inventory deduction, profit, taxes, invoices, multi-currency, crash recovery, AI/voice entry, and later modules are not implied by Increment 1 |
