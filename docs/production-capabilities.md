...
| Organizations, roles, and permissions | Better Auth organization server plugin plus organization/member/invitation schema and raw auth endpoints; ADR 0010 approves an organization-backed `businessId` boundary | Before product data, restrict organization creation, disable/intercept deletion, and implement the first owner workspace create/select/settings flow with fresh membership checks; a later multi-user brief must define Pisto roles/permissions and invitation delivery/acceptance/verification/denial/audit | No complete organization UI, invitation flow, Pisto RBAC model, admin/support role, or impersonation is claimed |
| Sales and business records | Approved product brief only; no sale, catalog, inventory, expense, cash, customer, supplier, or report schema/route/UI exists | Implement the approved typed-sale and previous-month-summary vertical slice with tenant ownership, money/time-zone rules, confirmation, idempotency, audit, correction, tests, and migration evidence | Inventory deduction, profit, taxes, invoices, multi-currency, offline writes, and later modules are not implied by the first slice |
...
| Voice input | No microphone UI, audio dependency, upload, transcription provider, or retention implementation | After text is proven, add visible push-to-talk, bounded authenticated upload, server transcription, editable transcript, deletion, accessibility, failure states, and physical-device evidence | No realtime speech-to-speech, always-listening/background recording, silent submission, or text-to-speech baseline |
